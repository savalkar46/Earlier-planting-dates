...
## This is a function that will use get the fraction of time the crop spends in a partiular temperature category
...
Data_AgGrids <- function(file_path){
  ...
  ## here we will read all the binary files for historical data
  df1 <-read_binary(paste0("/weka/data/project/agaid/rajagopalan_agroecosystems/commondata/meteorologicaldata/gridded/GCMs/US_Conus/", file_path), num_vars = 8) %>% 
  ...
  i=1
  j=1
  ## Base and cutoff temperature
  Tb = 0
  Tc = 30
  ## Planting dates that we have used for the study for historical we will use traditional planting dates
  ...
  ## We loop through all the planting dates here
  for (i in 1: n) {
	...
	## Identifying the growth stages of wheat based on the GDD accumulation
	...
	## All the SOL1 to SOL3 will be clubbed into SOL
	## Split as two lines for the same dataframe as the if_else has limit on number of conditionals
	...
	## here we find the fraction of time the crop spends in a particular temperature category
...
## Path to all the GCM's that we will use
models <- list.dirs("/weka/data/project/agaid/rajagopalan_agroecosystems/commondata/meteorologicaldata/gridded/GCMs/US_Conus/", full.names = FALSE, recursive = FALSE)

## files grid list
lat_lon_csv <- read.csv("/home/supriya.savalkar/Planting_Date/Scripts/East_Grid_List2.csv")
...
existing_file_paths <- file_path[file.exists(file.path("/weka/data/project/agaid/rajagopalan_agroecosystems/commondata/meteorologicaldata/gridded/GCMs/US_Conus/", file_path))]

df10<-rep()

## We run the function through grids here
df <- future_map_dfr(existing_file_paths, Data_AgGrids)

## This creates the database, OP stands for output and IP stands for input.
## files saved in RDS are lighter in size
saveRDS(df, paste0("/weka/data/project/agaid/supriya/Planting_Date/Future/Historical/Wheat/",state,".rds"))
