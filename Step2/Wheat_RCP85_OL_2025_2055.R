...
for (i in 1:length(Wheat)) {
  ...
  OP_Grid <- OP_Grid |> subset(Year>=2025 & Year<=2055)
  OP_Grid <- OP_Grid |> group_by(lat, lon, PD, Pheno_Stage, Temp_category, model, climate_proj ) |> 
    ...
    distinct()
  OP_Grid$Case <- "Wheat_Overlap_2025_2055"
  Masterfile2 <- rbind(Masterfile,OP_Grid)
  Masterfile <- Masterfile2
}
saveRDS(Masterfile, paste0("/weka/data/project/agaid/supriya/Planting_Date/Extracted/Wheat_RCP85_OL_2025_2055_V02.rds"))
