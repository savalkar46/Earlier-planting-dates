...
Wheat <- list.files("/weka/data/project/agaid/supriya/Planting_Date/Future/RCP85/Wheat")
Path <- "/weka/data/project/agaid/supriya/Planting_Date/Future/RCP85/Wheat"
Masterfile <- rep()
i=1
length(Wheat)
...
for (i in 1:length(Wheat)) {
  ...
  OP_Grid <- OP_Grid |> subset(Year>=2065 & Year<=2095)
  OP_Grid <- OP_Grid |> group_by(lat, lon, PD, Pheno_Stage, Temp_category, model, climate_proj ) |> 
    ...
    distinct()
  OP_Grid$Case <- "Wheat_Overlap_2065_2095"
  Masterfile2 <- rbind(Masterfile,OP_Grid)
  Masterfile <- Masterfile2
}
saveRDS(Masterfile, paste0("/weka/data/project/agaid/supriya/Planting_Date/Extracted/Wheat_RCP85_OL_2065_2095_V02.rds"))